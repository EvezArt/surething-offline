#!/usr/bin/env python3
"""
EVEZ Operator API — MCP server deployable to Vercel or run locally.
Run locally: uvicorn api.index:app --port 7777
Docs: /docs
...
def _normalize(obj: Any, float_paths: set[str], path: str = "") -> Any:
    ...
    if isinstance(obj, float):
        if math.isnan(obj) or math.isinf(obj): raise ValueError(f"Poison float at {path}")
...
def _make_event(event_type: str, lane: str, payload: dict, parent_hash: str | None) -> dict:
    ...
    base = {"event_type": event_type, "lane": lane, "payload": payload, "timestamp": ts,
            "height": len(_spine), "parent_event_hash": parent_hash or ""}
...
@app.get("/")
def root():
    return {"status": "ok", "service": "evez-operator-api", "version": "1.0.0",
            "spine_height": len(_spine), "docs": "/docs", "tools": "/tools"}

@app.get("/tools")
def list_tools():
    return {"schema_version": "1.0", "name": "evez-operator",
            "description": "EVEZ behavioral operating system tools",
            "tools": [
                {"name": "spine_event", "description": "Add typed event to EVEZ spine",
                 "inputSchema": {"type": "object", "required": ["event_type", "lane", "payload"],
                 "properties": {"event_type": {"type": "string", "enum": ["observation","transformation","claim","test","decision","linkage"]},
                 "lane": {"type": "string", "enum": ["empirical","documentary","metaphysical"]},
                 "payload": {"type": "object"}, "parent_event_hash": {"type": "string"}}}},
                {"name": "spine_verify", "description": "Verify SHA-256 chain integrity",
                 "inputSchema": {"type": "object", "properties": {"from_height": {"type": "integer", "default": 0}}}},
                {"name": "contradiction_check", "description": "List active unsat cores",
                 "inputSchema": {"type": "object", "properties": {"scope": {"type": "string", "default": "all"}}}},
                {"name": "fire_anchor_payload", "description": "Build Base Sepolia calldata for FIRE event",
                 "inputSchema": {"type": "object", "required": ["fire_number", "evidence_hash"],
                 "properties": {"fire_number": {"type": "integer"}, "evidence_hash": {"type": "string"},
                 "round": {"type": "integer"}, "v_score": {"type": "number"}}}},
                {"name": "canonicalize", "description": "Normalize + JCS + SHA-256 any JSON object",
                 "inputSchema": {"type": "object", "required": ["input"],
                 "properties": {"input": {"type": "object"}, "float_paths": {"type": "array", "items": {"type": "string"}}}}},
                {"name": "deploy_status", "description": "All EVEZ service health",
...
def add_spine_event(req: SpineEventRequest):
    ...
    if req.event_type not in VALID_TYPES: raise HTTPException(400, f"Invalid event_type: {req.event_type}")
    if req.lane not in VALID_LANES: raise HTTPException(400, f"Invalid lane: {req.lane}")
...
def get_contradictions(scope: str = "all"):
    cores = [
        {"id": "unsat-001", "scope": "revenue", "priority": "HIGH",
        ...
        {"id": "unsat-002", "scope": "fire", "priority": "MEDIUM",
        ...
        {"id": "unsat-003", "scope": "contacts", "priority": "LOW",
    ...
    if scope != "all": cores = [c for c in cores if c["scope"] == scope]
...
def fire_anchor(fire_number: int, evidence_hash: str, round: int = 0, v_score: float = 0.0):
    payload = {"evidence_hash": evidence_hash, "fire_number": fire_number, "round": round,
               "system": "EVEZ_OS", "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "v_score": v_score}
    jcs, digest = canonicalize(payload, float_paths=[".v_score"])
    calldata = "0x" + jcs.encode("utf-8").hex()
    return {"fire_number": fire_number, "anchor_hash": digest, "calldata": calldata,
            "calldata_length_bytes": len(jcs), "network": "base-sepolia"}
